...
class Hall:
    ...
    def list_rooms(self, player):
        ...
        if len(self.rooms) == 0:
            ...
            for room in self.rooms:
                msg += room + ": " + \
                    str(len(self.rooms[room].players)) + " player(s)\n"
            player.socket.sendall(msg.encode())
    ...
    def handle_msg(self, player, msg):
        instructions = b'Instructions:\n'\
            ...
            + b'[<quit>] to quit\n' \
            + b'[<json>] to sent json type data to server.\n'\
        ...
        if "name:" in msg:
            ...
            if(self.isJson(str)):
                str = str.replace('"', "'")
                print(str)
                player.socket.sendall(str.encode())
            else: 
                str = 'Please sent data by Json type. please resent again!'
                player.socket.sendall(str.encode())
            ...
            if player.name in self.room_player_map:
                self.rooms[self.room_player_map[player.name]
                           ].broadcast(player, msg.encode())
            else:
